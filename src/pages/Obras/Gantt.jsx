...
import { Plus, Trash2, X, PenTool, GripVertical, ChevronUp, ChevronDown } from 'lucide-react';
...
export default function Gantt({ obra, onClose }) {
  ...
  // +1 para incluir el último día (del 1 al 20 = 20 días, no 19)
  const totalDays = Math.max(Math.round((obraFin - obraInicio) / 86400000) + 1, 14);

  // Distribuir fases equitativamente en la línea temporal
  const distribute = (names, total) => {
    ...
    return names.map((name, i) => {
      ...
      const task = {
        id: Date.now() + i, name: typeof name === 'string' ? name : name.name,
  ...
  const [tasks, setTasks] = useState(() => {
    if (obra?.ganttTareas?.length) return obra.ganttTareas;
  ...
  const [editingDur, setEditingDur] = useState(null); // task id being edited

  // Redistribute all tasks keeping names, progress and colors
  const redistributeTasks = (taskList) => {
    let cursor = 0;
    const n = taskList.length;
    const durBase = Math.floor(totalDays / n);
    const remainder = totalDays - durBase * n;
    return taskList.map((t, i) => {
      const dur = durBase + (i < remainder ? 1 : 0);
      const result = { ...t, startDay: cursor, duration: dur };
      cursor += dur;
      return result;
    });
  };

  // Recalculate positions without changing durations
  const recalcPositions = (taskList) => {
    let cursor = 0;
    return taskList.map(t => {
      const result = { ...t, startDay: cursor };
      cursor += t.duration;
      return result;
    });
  };
  ...
  const handleAddTask = () => {
    ...
    const newTasks = [...tasks, { id: Date.now(), name: newTaskName, startDay: 0, duration: 1, progress: 0, color: COLORS[tasks.length % COLORS.length] }];
    setTasks(redistributeTasks(newTasks));
    setNewTaskName('');
  };

  const removeTask = (id) => {
    const remaining = tasks.filter(t => t.id !== id);
    if (remaining.length > 0) setTasks(redistributeTasks(remaining));
    else setTasks([]);
  };
  ...
  const updateDuration = (id, newDur) => {
    ...
    const updated = tasks.map(t => t.id === id ? { ...t, duration: dur } : t);
    setTasks(recalcPositions(updated));
  };
  ...
  return (
    <div className="modal-overlay" style={{ zIndex: 100 }}>
      <div className="modal-content" style={{ maxWidth: '96vw', width: '96vw', height: '90vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
          ...
          <div style={{ width: 'clamp(160px, 25vw, 260px)', flexShrink: 0, borderRight: '2px solid var(--border)', background: '#fff', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
            ...
            {tasks.length === 0 && <div style={{ padding: '24px 10px', fontSize: '11px', color: '#a1a1aa', textAlign: 'center' }}>Añade la primera fase →</div>}
            {tasks.map((task, index) => (
              <div
                key={task.id}
                onDragOver={handleDragOver}
                onDrop={() => handleDrop(index)}
                style={{ padding: '6px 8px', borderBottom: '1px solid var(--border)', background: dragIdx === index ? '#eff6ff' : (index % 2 === 0 ? '#fff' : '#fafafa'), height: '48px', display: 'flex', alignItems: 'center', gap: '6px', transition: 'background 0.15s' }}
              >
                <div draggable onDragStart={() => handleDragStart(index)} style={{ cursor: 'grab', flexShrink: 0, padding: '4px 2px', borderRadius: '3px', display: 'flex', alignItems: 'center' }} title="Arrastra para reordenar">
                  <GripVertical size={12} style={{ color: '#c4c4c4' }} />
                </div>
                <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: task.color, flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '10px', fontWeight: 600, color: 'var(--text-main)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{task.name}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '3px', marginTop: '1px' }}>
                    <input type="range" min="0" max="100" step="5" value={task.progress} onChange={(e) => updateProgress(task.id, e.target.value)} style={{ flex: 1, accentColor: task.color, height: '3px' }} />
                    <span style={{ fontSize: '8px', fontWeight: 700, color: task.color, width: '22px', textAlign: 'right' }}>{task.progress}%</span>
                  </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', flexShrink: 0 }}>
                  <button onClick={() => moveTask(index, -1)} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', color: index === 0 ? '#e4e4e7' : '#94a3b8', lineHeight: 0 }}><ChevronUp size={11} /></button>
                  <button onClick={() => moveTask(index, 1)} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', color: index === tasks.length - 1 ? '#e4e4e7' : '#94a3b8', lineHeight: 0 }}><ChevronDown size={11} /></button>
                </div>
                {/* Duration editor */}
                {editingDur === task.id ? (
                  <input type="number" min="1" max={totalDays} value={task.duration} autoFocus onChange={(e) => updateDuration(task.id, e.target.value)} onBlur={() => setEditingDur(null)} onKeyDown={(e) => e.key === 'Enter' && setEditingDur(null)} style={{ width: '32px', padding: '2px 3px', fontSize: '10px', textAlign: 'center', border: '1px solid var(--accent)', borderRadius: '4px' }} />
                ) : (
                  <button onClick={() => setEditingDur(task.id)} style={{ background: '#f1f5f9', border: '1px solid var(--border)', borderRadius: '4px', padding: '1px 4px', fontSize: '9px', fontWeight: 700, color: '#64748b', cursor: 'pointer', whiteSpace: 'nowrap' }}>{task.duration}d</button>
                )}
                <button onClick={() => removeTask(task.id)} style={{ background: 'none', border: 'none', color: '#f87171', cursor: 'pointer', padding: '1px', flexShrink: 0 }}><Trash2 size={10} /></button>
              </div>
            ))}
          </div>
          ...
          <div style={{ flex: 1, overflowX: 'auto', overflowY: 'auto', background: '#fff' }}>
            <div style={{ width: `${totalDays * dayW}px`, minWidth: `${totalDays * dayW}px` }}>
              ...
              {/* Timeline rows */}
              {tasks.map((task, index) => (
                <div key={task.id} style={{ height: '48px', position: 'relative', display: 'flex', borderBottom: '1px solid var(--border)', background: index % 2 === 0 ? '#fff' : '#fafafa' }}>
                  {realDays.map((day, i) => {
                    const isWeekend = day.weekday === 0 || day.weekday === 6;
                    const isHoliday = isFestivo(day.date);
                  ...
                  <div style={{
                    ...
                    <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${task.progress}%`, background: task.color, transition: 'width 0.3s' }} />
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', padding: '0 5px', color: task.progress > 50 ? '#fff' : task.color, fontSize: '8px', fontWeight: 700, whiteSpace: 'nowrap' }}>
                      {task.duration}d
                    </div>
                  </div>
                </div>
      ...
      {isPrintMode && <GanttPrint obra={obra} tasks={tasks} onClose={() => setIsPrintMode(false)} />}
    </div>
  );
}
